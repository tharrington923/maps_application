...
public class GraphAdjMatrix extends Graph {
	...
	/*
	Method to calculate the square of the matrix and then determine the 2 hop vertices 
	using squareAdjMatrix(). 
		
	public int[][] squareAdjMatrix(){
		int[][] squareAdjMatrix = new int[getNumVertices()][getNumVertices()];
		int sum;
		for(int i = 0; i < getNumVertices(); i++){
	...
	public List<Integer> getDistance2(int v){
		// Calculate the square of adjMatrix
		int[][] squareAdjMatrix = squareAdjMatrix();
	...
	public List<Integer> getDistance2(int v) {
		...
		for(int i = 0; i < dim; i++){
			sum = 0;
			for(int k = 0; k < dim; k++){
				sum += (adjMatrix[v][k]*adjMatrix[k][i]);
			}
			for(int j = 0; j < sum; j++){
				distanceTwo.add(i);
			}
	...
	/*
	Method to calculate the 2 hop distance vertices using getNeighbors.
